...
linewidth = 5
hollow = True
foldername = f"_centered_hollow={int(hollow)}_lw={linewidth}"
...
image_arrays = []
for s in range(51):
    image_array = nav.npload("/src", "data", "stimulus" + foldername, f"idx={s}_pxl=227.npy") # shape = (227, 227, 4)
...
nav.npsave(Rc, "/src", "results", "responses"+foldername+f"_light={int(light)}", f"CR_stim=shape_key={hidden_key}.npy")
