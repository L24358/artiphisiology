...
folders = [f"dyrange_{data_type}_key={hidden_key}_lw={linewidth}"]

Rs = []
for scale in [0.25, 0.5, 1, 2]:
    R = nav.npload("/src", "data", f"responses_{data_type}_hollow=0_lw={linewidth}_light={int(light)}_scale={scale}", f"CR_stim=shape_key={hidden_key}.npy")
