...
R_light = nav.npload("/src", "data", f"responses_{data_type}_hollow={int(hollow)}_lw={linewidth}_light=1_scale={scale}", f"CR_stim=shape_key={hidden_key}.npy")
R_dark = nav.npload("/src", "data", f"responses_{data_type}_hollow={int(hollow)}_lw={linewidth}_light=0_scale={scale}", f"CR_stim=shape_key={hidden_key}.npy")
folders = [f"onoff_{data_type}_key={hidden_key}_lw={linewidth}"]
